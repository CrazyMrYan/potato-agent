...
- 真正由本项目接管 MCP 和工具二次确认，需要后续切到 Pi SDK session 或本项目 `coding-agent-runtime`，在 `AgentOrchestrator -> Tool Boundary` 处拦截工具调用。
...
Trace 默认自动开启，不要求用户手动打开。它面向复盘、调试、回放和问题定位，记录任务输入、标准事件、工具摘要、最终状态、diff 和 runtime capability。用户通过 `agent trace` 或 TUI `/trace` 查看记录。
