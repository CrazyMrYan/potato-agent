...
| 目录 | 包名 | 职责 | 不放什么 |
| --- | --- | --- | --- |
| `protocol/` | `@potato/protocol` | 任务输入、事件、审批、diff、错误码等类型契约 | Pi 调用、事件映射实现、终端渲染、工具执行 |
| `core/` | `@potato/core` | `AgentGateway`、`AgentOrchestrator`、`PiAdapter`、事件映射、权限、trace、验证、工具边界 | CLI 参数解析、终端 UI |
| `cli/` | `@potato/cli` | 命令解析、交互输入、终端渲染、调用 core | 智能体编排策略、Pi 事件语义转换 |
| `wiki/` | 无 | 知识库和阶段记录 | 运行时代码 |

`AgentOrchestrator`、`PiEventMapper` 和 `PiRpcAdapter` 已从 CLI 拆入 `core/`。后续如果需要更强隔离，再从 `core/` 拆出独立 `potato-runtime` 子进程。
...
| 系统提示词、skills、MCP、工具 allow/deny、权限策略配置 | Core Config / AgentOrchestrator | 多宿主复用，后续 CLI、桌面端和 runtime 都应走同一套模型 |
| Pi RPC 启动、Pi 事件转换、Pi CLI 参数透传 | PiAdapter | 隔离 Pi 的具体 API 和运行形态 |
| 推理、规划、工具调用循环 | Pi | 复用 Pi 的底层 potato 能力 |
...
flowchart TB
    ...
    Runtime --> PiProc["potato-runtime 子进程"]
...
`potato run` 用于一次性任务验证。`potato chat` 用于保持同一个 Pi RPC 会话，在终端里做多轮输入，适合验证“前一轮上下文是否能延续到后一轮”。
...
- thinking delta 累积到下一个结构性事件前输出为一行，不额外套固定前缀。
- text delta 累积到下一个结构性事件前按正常段落输出。
- 工具结果输出压缩成单行摘要，避免长 JSON、文件内容和命令输出污染会话结构。
- TUI 使用 renderer 返回的结构化 kind 上色，不再依赖“步骤：”“工具开始：”这类文本前缀。
- `run` 和 `chat` 共用同一个 renderer，保证一次性任务和多轮会话展示一致。

### Core Config

`AgentConfig` 是 CLI、后续桌面端和 runtime 共享的运行配置模型。它不属于 protocol，因为它包含本地运行策略和宿主配置，不是跨进程稳定事件契约。
...
```text
只读工具 read、ls、grep、find 默认允许。
bash、edit、write 默认需要确认。
默认不启用完全放开模式。
```
...
Trace 默认自动开启，不要求用户手动打开。它面向复盘、调试、回放和问题定位，记录任务输入、标准事件、工具摘要、最终状态、diff 和 runtime capability。用户通过 `potato trace` 或 TUI `/trace` 查看记录。
...
Diff 是当前 Git 工作区变更视图，不等同于写入前审批。当前 RPC 路径只能在任务执行后展示 diff；写入前展示 patch 并确认，需要 SDK/runtime adapter 在工具执行前接入 `ToolBoundary`。
