...
第一阶段先使用 Pi SDK 已提供的 `RpcClient` 启动 Pi RPC 子进程。这样可以更早验证真实 Pi 路径，同时仍保留后续拆出独立 `potato-runtime` 仓库的能力。

## 工作区拆分

当前仓库 `potato` 是单一 Git 仓库，使用 pnpm workspace 管理模块。
...
coding-agent/
  wiki/      知识库和阶段记录
  protocol/  @potato/protocol，稳定协议和类型契约
  core/      @potato/core，AgentGateway、AgentOrchestrator、PiAdapter、工具边界
  cli/       @potato/cli，命令解析、终端交互和渲染
```

暂不单独拆 `potato-runtime`。等 Pi RPC 路径跑通并完成真实任务验证，再从 `core/` 抽出本项目 runtime 子进程。
...
它不应该直接调用 Pi。所有 Pi 相关能力必须经过 `@potato/core` 中的 `AgentOrchestrator -> PiAdapter`。
...
CLI 第一阶段在兼容命令 `potato run`、`potato chat` 中仍接收模型参数。M4 开始，默认 `potato` 入口进入 TUI，provider、model 和 API Key 可以在运行时输入。参数校验、供应商凭证映射和 `PiAdapterOptions` 组装由 `core/` 负责。这样后续桌面端可以复用同一套模型配置逻辑，而不是复制 CLI 实现。
...
```text
DEEPSEEK_API_KEY=... potato run "<任务描述>" --provider deepseek --model deepseek-chat
```

```text
potato run "<任务描述>" --provider deepseek --model deepseek-chat --api-key "$DEEPSEEK_API_KEY"
```
...
- `potato`、`potato run` 和 `potato chat` 默认使用 Pi。
...
```text
<workspace>/.potato/traces/<task-id>.jsonl
```
...
4. TUI 启动时确保生成 `<workspace>/.potato/config.json`。
5. 用户通过 `Ctrl+M` 打开模型配置选择器，用方向键选择 provider 和 model，并可输入 API Key；配置保存后写回 workspace 配置文件。
6. 用户输入任务后，TUI 通过 `@potato/core` 创建同一个 Pi RPC 会话。
...
```bash
potato chat --provider deepseek --model deepseek-reasoner --workspace <本地项目路径>
```
...
`potato chat` 是第一阶段验证多轮上下文的最小交互壳，不做复杂 TUI。
...
- `potato chat` 可以在同一个 Pi RPC 会话中连续发送多轮输入。
- `pnpm test` 通过。
- `pnpm typecheck` 通过。
- `pnpm build` 通过。
...
- `potato run` 和 `potato chat` 兼容命令继续可用。
...
- 每次 `potato run` 都产生 trace 文件。
- `potato trace` 可以读取最近任务。
- `potato diff` 能展示当前工作区变更。
...
- `potato run "<真实任务>"` 能通过 Pi 读取仓库并返回总结。
- CLI 能展示 Pi 进度和失败状态。
...
- 可以在一个本地 Git 仓库中运行 `potato run "<任务>"`。
- CLI 能展示任务进展。
- CLI 能展示 diff。
- 写文件和执行命令前能请求确认。
- 能运行验证命令并展示结果。
...
3. 保留 `potato run` 和 `potato chat` 兼容命令。
4. 在后续 M5 增加 trace/diff。
5. 在后续 M6 增加权限和工具边界。
6. 配置真实模型凭证，做端到端只读和小范围写入验证。
7. 评估是否从 `core/` 拆出独立 `potato-runtime` 子进程。
